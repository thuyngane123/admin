...
<div id="app">
    <div id="main">
        ...
        <div class="page-heading">
            ...
            <section class="section">
                <div class="card">
                    <div class="card-body">
                        <c:set value="${hocSinh }" var="hs"/>
                        <form action="${pageContext.request.contextPath}/UpdateHocSinhServlet" method="post">
                         
                            <input type="hidden" name="id" value="${hs.id != null ? hs.id : 0}" />
                            

                            <div class="mb-3">
                                <label for="hoTen" class="form-label">Họ Tên Học Sinh</label>
                                <input type="text" class="form-control" id="hoTen" name="ho_ten"
                                       value="<c:out value='${hs.hoTen}'/>" required>
                            </div>

                            <div class="mb-3">
                                <label for="gioiTinh" class="form-label">Giới Tính</label>
                                <input type="text" class="form-control" id="gioiTinh" name="gioi_tinh"
                                       value="<c:out value='${hs.gioiTinh}'/>" required>
                            </div>
                            
                            <div class="mb-3">
                                <label for="diaChi" class="form-label">Địa Chỉ</label>
                                <input type="text" class="form-control" id="diaChi" name="dia_chi"
                                       value="<c:out value='${hs.diaChi}'/>" required>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="sdtPhuHuynh" name="sdt_phu_huynh"
                                       value="<c:out value='${hs.sdtPhuHuynh}'/>" required>
                            </div>
       ...
       value="<c:out value='${hs.hoTenPhuHuynh}'/>">
    ...
    <select class="form-select" id="trangThai" name="trang_thai">
        <option value="Đang học" <c:if test="${hs.trangThai eq 'Đang học'}">selected</c:if>>Đang học</option>
        <option value="Đã nghỉ" <c:if test="${hs.trangThai eq 'Đã nghỉ'}">selected</c:if>>Đã nghỉ</option>
    </select>
</div>
							...
							<div class="mb-3">
    							...
    							<input type="date" class="form-control" id="ngayVaoHoc" name="ngay_vao_hoc"
           							value="${hs.ngayVaoHoc}" required>
							</div>
							<input type="hidden" name="id_nguoi_dung" value="${hs.idNguoiDung != null ? hs.idNguoiDung : ''}" />
                           ...
                           <button type="submit" class="btn btn-primary">
                            ...
                            <a href="${pageContext.request.contextPath}/HocSinhServelt" class="btn btn-secondary">Hủy</a>
                        </form>
                    </div>
                </div>
            </section>
